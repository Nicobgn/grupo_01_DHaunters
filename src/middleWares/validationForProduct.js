...
const validation = [
  ...
  body("img").custom((value, { req }) => {
    let file = req.file;
    let extencionesPermitidas = [".jpg", ".jpeg", ".npg", "png"];

    if (!file) {
      throw new Error(
        "Tienes que subir una imagen en formato jpg, jpeg, npg o png"
      );
    } else {
      let extencionDelArchivo = path.extname(file.originalname).toLowerCase()
      if (extencionesPermitidas.includes(extencionDelArchivo)) {
        throw new Error("Las extenciones permitidas son .jpg .jpeg .npg .png");
      }
    }
    return true;
  }),
